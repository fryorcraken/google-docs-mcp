...
import { register } from './listSmartChips.js';
...
describe('listSmartChips', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    captureToolExecute();
  });

  it('lists smart chips on a document with a single (legacy/synthetic) tab', async () => {
                    ...
                    elements: [
                      {
                        startIndex: 1,
                        endIndex: 2,
                        person: { personId: 'p1', personProperties: {} },
                      },
                    ],
                  },
                },
    ...
    expect(parsed.smartChips).toHaveLength(1);
  });
