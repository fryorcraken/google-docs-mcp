...
> **Fork notice.** This is a hard fork of [a-bonus/google-docs-mcp](https://github.com/a-bonus/google-docs-mcp) maintained by [@fryorcraken](https://github.com/fryorcraken). It adds Google Slides support, ships fixes for known upstream bugs, and is dual-licensed under MIT or Apache-2.0 (see [LICENSE-MIT](LICENSE-MIT), [LICENSE-APACHE](LICENSE-APACHE), and [NOTICE](NOTICE)). Original work is © Alain Bonus and upstream contributors.
...
| `GOOGLE_MCP_SCOPES`    | Comma-separated subset of `docs,sheets,slides,drive,gmail,calendar` to limit auth scopes and registered tools (default: all)                                                                                          |
| `MCP_LAZY_TOOLS`       | Set to `1` to expose only 3 meta-tools (`searchTools`/`describeTool`/`callTool`) at `tools/list`. Drops the per-session tool catalog from ~32k tokens to ~500. Tools are still callable via `callTool({name, args})`. |
