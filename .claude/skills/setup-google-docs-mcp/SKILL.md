...
3. **APIs & Services → Library**, enable each API the user wants:
   - Google Docs API
   - Google Sheets API
   - Google Drive API
   - Gmail API (only if Gmail tools needed)
   - Google Calendar API (only if Calendar tools needed)
4. **APIs & Services → OAuth consent screen**:
   ...
   - Scopes section: add `gmail.modify` if Gmail enabled, `calendar.events` if Calendar enabled. The Docs/Sheets/Drive scopes are added automatically by the server's auth request — don't pre-add them here.
...
In the MCP client, ask the agent to call `listGoogleDocs` (or similar harmless read tool). On success, the agent receives a JSON list of recent docs. On failure, the error message is usually descriptive — see Troubleshooting.
